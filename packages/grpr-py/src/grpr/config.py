...
def load_config(*, cwd: Optional[str] = None, config_path: Optional[str] = None) -> LoadedConfig:
    ...
    explicit_config = config_path or os.environ.get("GRPR_CONFIG_PATH")
    if explicit_config:
        root_dir = str(Path(explicit_config).resolve().parent)
    else:
        root_dir = find_repo_root(working_dir)

    resolved_config_path = Path(explicit_config) if explicit_config else Path(root_dir) / ".grpr.json"
