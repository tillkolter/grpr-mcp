...
import { GrprEvent, GrprLevel } from "grpr-core";
import { emit } from "grpr";
import { startMcpServer } from "grpr-mcp";

const printHelp = (): void => {
  console.log(`Grpr - MCP-first telemetry\n\nCommands:\n  init                 Create .grpr.json and .grpr.local.json\n  checkpoint           Write a .grpr-checkpoint epoch timestamp\n  wrap --service <s> --session <id> -- <cmd...>\n                       Capture stdout/stderr and write JSONL\n  emit --service <s> --session <id>\n                       Read JSON events from stdin and append\n  mcp                  Start MCP server\n`);
};
...
const writeInitFiles = (rootDir: string): void => {
  const configPath = path.join(rootDir, ".grpr.json");
...
const toLevel = (type: "stdout" | "stderr"): GrprLevel =>
  type === "stderr" ? "error" : "info";

const buildEvent = (
  type: "stdout" | "stderr",
  message: string,
  base: Pick<GrprEvent, "service" | "run_id" | "session_id">,
): GrprEvent => {
  return {
    id: randomUUID(),
...
const handleWrap = async (argv: string[]): Promise<number> => {
  ...
  if (!config.enabled) {
    console.log("Grpr disabled via config/env; running command without capture.");
  ...
  const runId = process.env.GRPR_RUN_ID ?? randomUUID();
  const service = opts.service ?? config.default_service;
  const session = opts.session ?? process.env.GRPR_SESSION_ID;
  ...
  const child = spawn(rest[0], rest.slice(1), {
    ...
    env: {
      ...
      ...(session ? { GRPR_SESSION_ID: session } : {}),
    },
  });
...
const handleEmit = async (argv: string[]): Promise<number> => {
  ...
  for (const item of items) {
    if (item && typeof item === "object") {
      const event = item as Partial<GrprEvent>;
